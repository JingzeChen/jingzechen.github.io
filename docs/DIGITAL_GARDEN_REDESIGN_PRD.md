...
| 项目 | 内容 |
| --- | --- |
| 文档状态 | Historical baseline v1.1 + current status appendix |
| 创建日期 | 2026-08-04 |
| 产品形态 | 个人网站 / Digital Garden / 长文知识库 |
| 目标平台 | Desktop、Tablet、Mobile Web |
| 当前技术栈 | Jekyll 4、Chirpy、Liquid、SCSS、原生 JavaScript |
| 主要语言 | 英文界面、中文与英文内容 |
| 关联规范 | [POST_GUIDE.md](POST_GUIDE.md)、[CONTENT_QUALITY_REPORT.md](CONTENT_QUALITY_REPORT.md) |

## 1. 文档目的

本文档定义 Jingze's Garden 下一阶段的产品定位、信息架构、视觉系统、页面结构、交互规则、响应式行为、质量标准和实施顺序，用于指导设计、开发、内容维护与验收。

> **文档时间边界**：第 1 至 19 节保留 2026-08-04 改版启动时的线上走查、问题判断和设计目标，
> 其中 `194 篇`、`约 14 个 Series` 等数字是历史验收基线，不代表当前库存。当前可计算状态以
> [CONTENT_QUALITY_REPORT.md](CONTENT_QUALITY_REPORT.md) 为唯一来源；第 19 节末尾的“实施状态”
> 记录截至 2026-08-23 的结果。
...
- 让当时 194 篇以上的阅读笔记从“可浏览的文章列表”升级为“可检索、可分组、可连续阅读的个人知识库”。
...
改版启动阶段应更诚实地表达为 **Reading-first Digital Garden**：先把当时 194 篇读书笔记和约 14 个系列组织好，再随真实内容增长逐步开放 Notes、Essays、Projects 等路径。界面不提前展示尚不存在的产品形态。
...
- 当前库存为 450 篇：439 篇 `reading`、11 篇 `podcast`，共 28 个 Series；模板 Description、
  重复 Description 和隐藏 Topic 均为 0，构建门禁阻止回归。
- 单一 Status 降噪已覆盖首页精选、Series、关联内容和通用索引；文章页保留自身成熟度，列表标记只在至少两种可信状态存在时启用。
- Series 已集中补齐作者并提供章节 `uid` 锚点；文章元数据和 Series 导航可定位当前章节。
- `tools/content-quality.rb` 提供可重复的数据盘点与启用门槛报告；当前 213 篇超过 120 个 TOC 项，已列入人工编辑复核清单。
- Topic 共现边已增长到 22，Knowledge Map 数据门槛已满足；是否增加图形增强仍需服从性能、可访问性和真实阅读价值。
- Podcast 使第二种 Content Type 形成，Cross-type Library 数据门槛已满足；Status 筛选仍因 450 篇全部为 `growing` 而关闭。
- `updated` 仍集中在迁移批次，Latest Changes 继续等待编辑审计，不用自动生成日期制造活跃度。
- GoatCounter 匿名聚合统计、站点体积预算和定时线上健康检查已经接入；低光主题仍是后续产品决策。
