# Reading Input: Lecture 17 - Cache Consistency - Memcached at Facebook
...
- Part 18: Lecture 17 - Cache Consistency - Memcached at Facebook (https://www.bilibili.com/video/BV16f4y1z7kn/?p=18)

## Resources

### paper: Memcached at Facebook (2013)

- URL: http://nil.csail.mit.edu/6.824/2021/papers/memcache-fb.pdf
- Local path: official-materials/papers/memcache-fb.pdf
...
- URL: http://nil.csail.mit.edu/6.824/2021/papers/memcache-faq.txt
- Local path: official-materials/faqs/memcache-faq.txt
...
Yes, indeed.  It is something FB has struggled with because it makes
...
inconstency, they probably won't realize it and don't care.  (Their
target applications are not banking applications.)

Q: How does the MySQL replication system work?

See https://dev.mysql.com/doc/refman/8.0/en/replication.html.  FB uses
the log-based replication scheme as a component for publish/subscribe
system, as described in "Wormhole: Reliable Pub-Sub to support
Geo-replicated Internet"y Services, Sharma et al, 2015.  The core of
the replication scheme is to read updates from MySQL's transaction log
and send those to the backup, which applies them to its data.

Q: What exactly does "look-aside" caching refer to?

The cache sits on the side as opposed in between the application and
the storage layer.  If the application misses in the cache, the
application retrieves the database records and updates the cache,
instead of the cache doing it.

### homework: Question

- URL: http://nil.csail.mit.edu/6.824/2021/questions.html?q=q-memcached&lec=17
- Local path: official-materials/questions/17-q-memcached.html
