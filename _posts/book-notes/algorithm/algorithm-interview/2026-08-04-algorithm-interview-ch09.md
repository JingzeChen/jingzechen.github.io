...
description: "从完全二叉树的局部偏序推导建堆、上滤和下滤，说明优先队列在动态最值、Top-k、多路归并及惰性删除中的取舍。"
toc: true
...
考虑全局最优团队中效率最小的工程师，其效率为 $e^{\ast}$。处理到 $e^{\ast}$ 时，最优团队所有成员都已进入前缀。堆选择前缀中速度和最大的至多 $k$ 人，因此其速度和不小于最优团队速度和；与 $e^{\ast}$相乘得到的候选不会小于最优值。另一方面，堆中团队实际最小效率至少为 $e^{\ast}$，其真实性能不小于该候选，所以候选不可能超过全局可实现最大值。两者合起来说明必取到最优值。
