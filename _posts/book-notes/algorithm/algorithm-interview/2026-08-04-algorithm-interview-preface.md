...
- **输入**：算法处理的对象。例如排序算法的输入可以是一个含若干可比较元素的序列。
...
例如，一个方法依赖输入有序，那么遇到无序输入时至少要追问：能否先排序？排序会不会破坏原下标信息？排序的预处理成本是否满足约束？是否存在不排序的方法？只有能回答这些问题，才算把方法从原题迁移到变式。
...
输入规模变大时，时间复杂度描述基本操作次数的增长上界。大 O 记号关注增长趋势，而不把机器速度、单次操作成本等常数因素当成算法本身的差异。

例如，一个双重循环的执行次数同时包含平方增长部分、线性增长部分和固定部分。当输入足够大时，平方增长部分主导整体趋势，因此其时间复杂度记为 O(n²)。这不只是机械地“删掉低阶项”，而是要确认从某个规模开始，操作次数始终受某个平方增长的上界约束。

额外空间复杂度描述算法除输入本身以外所需存储随输入规模的增长趋势。分析时必须说明是否把输入、输出、递归调用栈计入空间，否则两个结论可能采用了不同口径。
...
方法一在无重复元素时会逐一检查所有元素对，比较次数随输入规模呈平方增长，因此最坏时间复杂度为 O(n²)，额外空间为 O(1)。

方法二最多对每个元素各做一次集合查询和一次插入。在哈希分布良好、集合操作平均为 O(1) 的假设下，平均时间为 O(n)；集合最多保存全部元素，额外空间为 O(n)。这里必须保留“平均”和哈希假设，不能把它无条件说成所有情况下的严格最坏 O(n)。

当输入规模翻倍时，逐对比较的工作量约变为原来的四倍，而集合方法的工作量约变为两倍。代码中的计时趋势与操作计数相呼应，但不构成严格证明；复杂度来自操作计数和假设，基准测试只是经验验证。
