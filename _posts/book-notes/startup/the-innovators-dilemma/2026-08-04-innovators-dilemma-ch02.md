...
$$
S_0(1+g_s)^{t^{\ast}}=R_0(1+g_r)^{t^{\ast}}
$$

整理并取对数：

$$
t^{\ast}=\frac{\ln(R_0/S_0)}{\ln((1+g_s)/(1+g_r))}
$$
...
$$
F_H+v_Hq^{\ast}=F_F+v_Fq^{\ast}
$$

因此：

$$
q^{\ast}=\frac{F_H-F_F}{v_F-v_H}
$$

当 $q<q^{\ast}$，低固定成本的闪存可能更便宜；当 $q>q^{\ast}$，硬盘的单位容量优势占主导。这解释了闪存为何先进入 10 至 40MB 等低容量用途，而不是直接挑战 350MB 笔记本市场。
