...
> 本文按照原书前言的顺序整理。前言本身不讲具体公式或算法，它的任务是说明全书研究什么、如何研究、适合谁读，以及第二版为何重写。
...
先不要给所有候选系统简单加权打分，而应先排除不能满足硬约束的方案。
...
只有满足硬约束的方案，才适合比较性能、可维护性、成本效率和实施风险等软目标。“视情况而定”的真正含义是：变化的不只是候选技术的能力，**场景决定的权重和风险承受能力也在变化**。同一种技术在两个团队中可能得到不同结论，因为一个团队已经具备成熟运维经验，另一个团队还没有。
...
更稳妥的做法是同时观察 **Pareto 前沿**。若方案 A 在所有关键指标上都不差于方案 B，并且至少一项更好，就说方案 A 支配方案 B；被支配的方案通常可以淘汰。剩余方案之间没有绝对胜者，只能依据场景选择。这正是作者所说“没有一种方法适合所有情况”的直觉。
...
高可用性关注尽量减少不可用时间。可用率越高，同一观察窗口内允许的不可用时间就越短。
