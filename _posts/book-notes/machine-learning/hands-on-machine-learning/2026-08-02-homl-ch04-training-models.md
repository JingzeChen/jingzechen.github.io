...
无论 $\lvert \theta\rvert$ 多小，梯度幅度始终为 1，所以参数以近似恒定速度向 0 推进。相比之下，Ridge 的梯度 $2\theta$ 会随参数变小而减弱，通常只会趋近 0，不会精确到 0。

图 4-19 中，高阶 Lasso 在适当 $\alpha$ 下近似三次曲线，因为更高阶特征的权重被压到零。

#### 不可微问题

$\lvert \theta\rvert$ 在 0 处左导数为 $-1$、右导数为 $+1$，普通导数不存在。可使用次梯度。
...
严格地说，$\lvert \theta\rvert$ 在 0 处的次梯度集合为整个区间 $[-1,1]$。公式选择 0 作为一个合法次梯度。
...
| Lasso 在 0 处有普通梯度 | $\lvert \theta\rvert$ 在 0 处不可微，应使用次梯度 |
