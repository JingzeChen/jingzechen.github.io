...
> 代码验证（6.5 节）：用 IIS 求得最优解后，检查 $\lvert E_P(f_i)-E_{\tilde P}(f_i)\rvert$，结果为 $5.6\times10^{-17}$、$0$、$0$——约束在机器精度内精确成立。
...
2. 计算 $g_k=g(w^{(k)})$。若 $\lVert g_k\rVert<\varepsilon$，停止，得 $w^{\ast}=w^{(k)}$；
...
**输出**：$w^{\ast}$ 及模型 $P(Y=1\mid x)=\sigma(w^{\ast}\cdot x)$
...
4. 若 $\lVert g_k\rVert<\varepsilon$，停止，输出 $w^{\ast}=w^{(k)}$；
5. 更新
