...
$$
\boxed{\;
f^{\ast}(x)=\frac12\log\frac{P(y=1\mid x)}{P(y=-1\mid x)}
\;}
$$
...
**（2）$\operatorname{sign}(f^{\ast})$ 是贝叶斯最优分类器。**
...
例如绝对损失 $\lvert y-f\rvert$、Huber 损失等，每步的
...
14. 指数损失的最优解是 $f^{\ast}(x)=\frac12\log\frac{P(y=1\mid x)}{P(y=-1\mid x)}$，即**对数几率的一半**。
...
| $f^{\ast}$ 估计 | — | $\frac12\log\frac{p}{1-p}$ | $\log\frac{p}{1-p}$ |
