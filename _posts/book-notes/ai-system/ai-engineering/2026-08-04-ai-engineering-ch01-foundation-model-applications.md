...
有限词表可以组成近乎无限的序列。若词表大小为 $\lvert \mathcal V\rvert$、最大输出长度为 $L$，候选序列数为
...
$$
N^{\ast}=\frac{F}{c_{\text{api}}-c_{\text{self}}}
$$
...
| 问题 | 简化公式 | 使用边界 |
|---|---|---|
| buy-or-build | $N^{\ast}=F/(c_{\text{api}}-c_{\text{self}})$ | 还要计入质量、闲置、团队与迁移成本 |
