...
description: "从训练数据、Transformer、模型规模、后训练与采样机制出发，理解基础模型的能力、成本、偏差和生成风险如何形成。"
toc: true
math: true
mermaid: true
---
...
模型规模主要由层数 $L$、隐藏维度 $d$、前馈维度 $d_{ff}$、词表大小 $\lvert V\rvert$ 以及头配置决定。以下维度展示了规模如何增长：
...
$$
N^{\ast}=\sqrt{\frac{C}{120}},
\qquad
D^{\ast}=20N^{\ast}.
$$

算力增加四倍，计算最优参数量才增加约两倍，因为 $N^{\ast}\propto\sqrt C$。
...
| $D^{\ast}\approx20N^{\ast}$ | Chinchilla 数据中的计算最优经验比例 |
