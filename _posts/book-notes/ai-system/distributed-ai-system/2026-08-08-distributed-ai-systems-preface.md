...
> 本笔记严格沿序言的叙述顺序展开：先解释大模型为什么把 AI 变成分布式系统问题，再说明本书的目标读者与十一章路线，最后整理实践环境、配套资源、排版约定和反馈渠道。序言本身不包含算法推导；文中的数值例子是为了把序言提出的问题讲透，不应误认为原书序言直接给出的内容。具体框架接口仍应以对应正文和配套仓库为准。
...
仅加载权重所需的理想内存，可以用模型参数量乘以每个权重的平均字节数快速估算。例如，BF16 或 FP16 权重通常占 2 字节：
...
“仅权重”绝不是训练总显存。混合精度 Adam 训练还要保存梯度、一阶矩、二阶矩以及可能存在的 FP32 主权重。若这些状态合计按每参数约 16 字节估算，7B 模型的模型状态约需 112 GB，70B 约需 1.12 TB，且仍未包含激活值、通信桶、临时工作区、内存碎片和框架开销。这不是所有训练配置的精确常数：优化器、精度、是否保留主权重以及实现方式都会改变它。它的用途是快速判断数量级，避免把“14 GB 的 7B 权重”误解为“14 GB 就能训练 7B 模型”。

训练峰值显存还要综合模型状态、激活、临时张量和碎片。不同部分对应不同手段：
...
如果多个工作进程各自训练模型副本却从不交换信息，它们得到的是多个不同模型，而不是一个更快训练完成的共同模型。为了保持一个逻辑训练任务，进程必须在某些边界交换状态。

以同步数据并行为例，每个 rank 处理不同的局部批次并得到局部梯度，再共同求出全局平均梯度。
...
分布式训练的一步耗时包括 GPU 有效计算、梯度或参数通信、数据准备以及不同 rank 速度不一致造成的等待；计算与通信重叠可以隐藏其中一部分时间。

一次通信的耗时同时受启动或同步延迟、传输数据量和链路有效带宽影响。
...
若先忽略激活值和临时缓冲区，DDP 每卡仍保留完整模型状态；理想完全分片时，FSDP 将每卡持久模型状态降为总状态的一部分。
...
批处理将多个请求合并执行，提高 GPU 利用率并摊薄固定开销。请求吞吐量看单位时间完成的请求数，token 吞吐量看单位时间生成的 token 数。

但等待更多请求凑批会增加排队延迟。一个用户请求的端到端延迟包括排队、prefill、decode、网络和运行时开销。增大批次可能降低单位 token 成本，却提高排队时间；不同长度请求放在固定批次中，短请求还可能等待长请求。于是服务系统需要连续批处理、抢占或更细粒度调度，而不能只把离线静态 batching 原样搬到线上。
...
对稠密 Transformer，可用参数量与训练 token 数估算训练计算量。前向和反向传播会让每个参数经历多次乘加工作；这不是适用于所有架构和算子的精确常数，但能用于容量规划。

设备数量、单设备峰值算力和端到端实际利用率共同决定理想训练时间。实际利用率还受到内存访问、通信、数据加载、气泡和软件开销影响，直接使用厂商峰值 FLOPS 会严重低估训练时间。

#### 3.1.2 数据准备为什么属于分布式基础

多 rank 训练时，每个 rank 应得到不同的数据子集，同时所有子集共同保持预期的数据分布。若每个 rank 都读取完整数据，计算量虽然随 rank 数增加，有效样本覆盖却没有相应增加；若切分遗漏或重复，训练语义也会改变。
...
框架可以隐藏 kernel 启动和通信调用的细节，却不能消除物理约束。一个算子可能受计算单元限制，也可能受显存带宽限制。算术强度比较浮点运算量与内存数据移动量；Roofline 模型据此判断性能上界更接近峰值计算吞吐还是显存带宽限制。

当内存带宽先成为上限时，继续增加理论计算单元不会直接解决瓶颈；应减少内存移动、融合算子或提高复用。分布式环境还多出一层网络 Roofline：一个 rank 计算得再快，也可能在等待参数、梯度、激活或 token 路由。
...
混合并行就是同时沿多个正交维度切分，例如让数据并行、张量并行和流水线并行的规模共同组成训练 world size。在具体系统中还可能加入上下文或专家维度，但并非所有维度都能无条件组合；专家并行可能与数据并行复用进程组定义。重点是每个 rank 同时属于若干不同通信组，必须明确每一组负责哪类交换。
...
全局有效批量由每个 rank 的局部批量、数据并行 rank 数和每次更新前累积的 microbatch 数共同决定。

从 1 卡扩到 8 卡而保持局部批量不变，会使全局有效批量扩大 8 倍。这样即使代码可运行，优化轨迹也不再与单卡基线等价；学习率、warmup、正则化和总更新步数可能都需重新考虑。DDP 的系统正确不自动保证实验语义不变。

#### 3.3.3 为什么使用梯度桶与重叠

若等到整个反向传播结束才同步全部梯度，通信与计算串行，step 时间近似为二者之和。DDP 把参数梯度组成 bucket；当某个 bucket 中的梯度全部就绪，就在后续层仍进行反向计算时发起通信。

理想重叠后，一步耗时主要由反向计算和 AllReduce 中较慢的一方，再加上无法隐藏的开销决定，而不是二者简单相加。bucket 太小会产生大量启动延迟，太大又会推迟第一次通信；因此 bucket 大小是计算图、网络特征和重叠机会之间的权衡，不能只凭一个固定推荐值。
...
FSDP 的关键不是简单地把模型切成若干份后各算各的，而是让每个 rank **持久保存自己的状态分片，在执行某个单元时临时收集所需完整参数**。一个典型生命周期是：
...
以列并行线性层为例，权重按输出维切分后，每个 rank 计算其中一块，最后按后续算子需求拼接或继续分片。这个分解来自矩阵分块规则；其性能是否好，则取决于输入复制、输出聚合和后续层通信能否有效组织。

#### 3.5.4 流水线为什么会有气泡

把模型切成多个 stage 后，一个 microbatch 必须依次经过各 stage。若一次迭代只有很少 microbatch，部分 stage 会在流水线填充和排空期间空闲。增加 microbatch 可以降低气泡比例，但会影响调度、激活存储和有效批量；stage 计算不均衡时，最慢 stage 还会成为节拍瓶颈。实际训练还包含前向与反向调度，但“增加 stage 会引入填充、排空与平衡问题”的直觉不变。
...
- **Decode** 每轮只为每个活跃序列生成下一个 token，要按生成长度重复多轮，常受 KV cache 读取和调度开销限制。
...
对仅作数量级理解的 decoder-only Transformer，单序列 KV cache 随层数、序列长度、每个 token 的 KV 表示宽度和元素字节数共同增长；Key 与 Value 都要计入。多个请求同时驻留时，还要累加各请求的序列长度。

不同注意力结构会改变 KV 表示宽度，分页分配会增加块级开销，因此这仍是估算思路。它解释了一个关键事实：模型权重固定不变，服务显存却会随活跃请求数和上下文长度动态变化。
...
SGLang 的 RadixAttention 用 radix tree 一类前缀结构组织可复用 KV cache。命中一部分前缀缓存后，本次只需重新 prefill 未命中的剩余部分。
...
解析器根据当前语法状态确定每一步允许采样的词表子集。这样减少格式失败与重试，但不能保证字段内容在业务上真实或正确。**语法正确、模式正确和语义正确是三个层次。**
...
加速比比较多设备与单设备的性能，强扩展效率再把加速比除以设备数。若 8 卡吞吐是单卡的 6.4 倍，扩展效率就是 80%。损失来自通信、同步、负载不均和额外调度。若模型单卡放不下，此时应选最小可行配置作为基线，并清楚说明效率定义。

Amdahl 定律进一步说明固定问题规模下，即使设备持续增加，不可并行部分仍限制总加速。这是为什么优化应先找主导串行段和通信段，而不是机械增加 GPU。

#### 3.10.4 Profiling 为什么必须和正确性验证并列

减少同步、改变精度、调整 batch 或缓存都可能提高速度，却也可能改变训练收敛、输出质量或请求语义。有效优化必须在提升性能的同时守住最低质量要求。
...
Mixture of Experts 让路由器为每个 token 只选择少数专家。模型总容量可以随专家总数增长，而每个 token 的专家计算主要取决于实际激活的少数专家。
...
第 1 节的估算只覆盖权重和模型状态，不包含激活、临时缓冲与碎片，因此结果是下界而不是采购结论。以 2 字节权重和每参数 16 字节训练状态粗估，7B、70B 和 1000B 模型仅权重约需 14 GB、140 GB 和 2000 GB，训练状态则约需 112 GB、1120 GB 和 16000 GB。这里使用十进制 GB，激活和 KV cache 还会随 batch、层数、宽度、序列长度和注意力结构变化。
...
即使忽略容量，梯度同步也很重。若 BF16 梯度大小约为 140 GB，8-rank ring AllReduce 中每个 rank 每步发送的数据量约为 245 GB。

接收量也在同一数量级。实际库会采用分块、多通道和拓扑感知算法，且通信可与反向计算重叠；这个估算只说明大模型并行策略必须认真考虑链路，而不能把 AllReduce 当作零成本函数调用。

### 11.3 为什么理想 8 卡完全分片仍然不够

若训练状态理想均匀分到 8 卡，每卡持久状态下界仍为 140 GB，超过 80 GB，而且尚未加入激活与临时完整层参数。因此，总显存 640 GB 也小于 1120 GB；任何只在这 8 张卡的 HBM 中保存全部训练状态的方案都不可能成立。

这一步排除了“只打开 FSDP 就能解决”的错误方案。候选方向变为：

1. 增加 GPU 数量；仅按状态下界就至少需要 14 张，实际还要为激活和临时峰值留余量。
...
4. 一个 4-stage 简化流水线使用 16 个 microbatch，按前述简化方法估算的利用率是多少？
