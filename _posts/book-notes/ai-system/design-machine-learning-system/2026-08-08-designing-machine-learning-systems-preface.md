...
>
> 本文按照原书前言的顺序展开。前言本身没有提出具体算法，它的主要任务是说明：为什么机器学习系统设计不能脱离上下文、全书为谁而写、讨论与不讨论什么，以及各章如何覆盖机器学习项目生命周期。文中标为“分析框架”“背景复习”或“教学示例”的数字和示例，是为了把前言的方法论变成可操作的工具，不应误认为作者在前言中直接给出的模型。
...
先排除违反正确性、合规性、延迟上限等硬约束的方案。违反隐私规定或延迟上限的模型，不能靠更高的准确率“抵消”。

对剩余方案，再结合当前上下文比较业务价值、模型质量、开发速度、可维护性、成本和风险。不同组织对这些软目标的权重以及对实施、运行和认知不确定性的敏感程度并不相同。
...
因此，这个框架的价值不是自动算出答案，而是迫使团队显式写出假设、约束和证据。

### 1.4 教学示例：怎样判断是否从批量预测切换到在线预测

原文列出的具体问题是：某位工程师相信在线预测会提高模型表现，但不知道怎样说服经理。这个问题不能只说“在线更新鲜，所以更好”，因为经理需要的是**可验证的增量价值与完整成本**。

每月净增量价值应同时计算推荐机会、转化率增量、每次新增转化的贡献价值，以及在线运行、建设摊销、可靠性、值班和迁移风险等成本。

假设每月有 200 万次推荐机会，在线预测带来的转化率增量为 0.2%，每次新增转化贡献 25 元，三类成本分别为 1.8 万、3 万和 1.2 万元，则每月净增量价值为 4 万元。若真实增量只有 0.05%，新增价值就只有 2.5 万元，净值变成负 3.5 万元。可见结论对转化率增量极其敏感，而这个量恰好不能靠主观确信获得。
...
4. 用小流量 A/B 测试估计转化率增量及置信区间。
5. 把服务成本、故障代价和人员投入一起纳入决策。
6. 只有在证据支持时再逐步扩大流量，并保留回滚路径。

这项估算还依赖多项简化假设：转化增量能归因于预测方式、每次转化价值近似稳定、实验不存在严重干扰、长期用户行为不会反转。现实中必须通过实验设计和敏感性分析校正，而不是把估算表当成事实。

### 1.5 教学推导：为什么“多久重训一次”也没有固定答案

重训间隔应同时考虑每次训练的成本，以及模型随数据变化而过时造成的损失。若训练越昂贵，分摊成本需要更长间隔；若环境变化越快，等待更新造成的平均损失增长越快。
...
这个判断不是生产系统的万能调度器。它假定每次训练能完全恢复模型、漂移近似线性、训练成本恒定、更新没有验证和发布延迟。突发事件、标签延迟、监管门禁、季节性和训练失败都会破坏这些假设。它真正说明的是：**重训频率应由变化速度、业务损失和更新成本共同决定，而不是套用“每天一次”之类的经验常数。**
...
模型通常可以抽象为一个由训练参数决定、把输入映射为预测的计算核心。这个抽象没有说明输入如何产生、输出怎样被使用，也没有处理部署、反馈和责任。
...
传统手写程序主要由代码显式规定行为；机器学习模型的行为还由训练数据共同决定。训练过程让模型在经验数据上减少预测损失，部署后真正关心的则是它在生产数据上的表现。

训练有效通常隐含一个关键前提：训练样本与部署时的数据分布足够接近。二者不一致时，即使训练和测试代码完全没有 bug，生产效果也可能下降。这解释了为什么数据分布变化是系统级问题，而不只是模型算法问题。
...
最终业务效用由预测质量、延迟、可用性、成本、安全与合规、人的使用行为和外部环境共同决定，而且这些因素通常具有非线性交互：
...
因此，不能简单假定预测质量提高就一定会带来业务价值提升。系统设计的目标不是让每个局部指标分别达到最高，而是在硬约束下找到可运行、可维护、能产生净价值的整体方案。
...
是否值得建设平台，需要比较平台建设与各团队迁移成本，以及各自重复建设和不一致造成的事故、审计与维护成本。
...
逻辑回归先计算线性打分，再通过 sigmoid 把结果建模为条件概率。仅仅落在 0 到 1 区间并不足以赋予一个数概率语义；输出与真实发生频率是否一致，还取决于数据、模型设定、拟合和概率校准。逻辑回归常是重要基线：若复杂模型不能在可信评估上稳定超过它，就没有理由承担额外复杂度。

矩阵分解式协同过滤常把用户和物品映射到低维向量，以向量匹配程度表示潜在偏好。但观测交互并不是随机样本：用户只能点击被曝光的商品，因此模型容易把既有曝光政策误认为自然偏好。

Transformer 的缩放点积注意力根据查询与键的匹配程度，对值进行加权聚合。
...
损失衡量单个预测与目标的差异，训练通常同时减少平均损失，并通过正则项惩罚不希望出现的复杂度。正则强度控制拟合与约束之间的权衡。

业务指标、评估指标和训练损失不是同义词。业务目标可能是利润或用户留存，评估指标可能是 F1，训练损失可能是交叉熵。需要通过实验验证三者之间的代理关系。

#### 梯度下降

梯度下降沿当前目标下降最快的局部方向更新参数。它有效的依据是局部线性近似；学习率过大时近似失效，目标可能震荡或发散。
...
ROC 曲线在不同阈值下比较真阳性率与假阳性率。

ROC-AUC 可解释为随机抽取一个正例和一个负例时，模型把正例排在负例之前的概率。严重类别不平衡时，还应检查 Precision–Recall 曲线和具体业务阈值。

回归任务常用均方误差，它会显著放大大误差，适合大偏差确实更昂贵的场景；若数据有强离群点，它也可能被少数样本支配。

似然表示给定模型参数时观测数据出现的可能性。独立样本下，对数似然把各样本概率的连乘转换为对数概率之和。
...
概率用于表达不确定性。方差衡量数据围绕均值的离散程度。训练数据变化导致模型表现大幅变化时，也常说估计具有高方差。

#### 正态分布与长尾分布

正态分布由均值和方差决定。它对称、尾部衰减快，适合许多聚合噪声的近似。现实 ML 数据经常呈长尾：少数类别、用户或物品占据大量交互，大量对象只出现很少次数。长尾会带来：
...
生产效果下降可能来自生产数据分布变化，即使代码与参数完全未改。反过来，检测到某个特征分布变化，也不一定意味着业务质量下降；必须结合影响路径和标签验证。
...
例如，排序质量提升可能让用户更快发现相关商品，继而增加有效转化和净贡献。
