...
本笔记严格沿原文段落顺序展开。为把作者的短论述讲透，文中会加入示例和图表。这些内容均是**辅助理解的教学化补充**，不是作者在 FORWARD 中给出的固定模板。
...
设计取决于目标、负载、约束和优先级，而不是题目名称。只要这些条件变化，合理架构就可能变化。背下“新闻信息流标准答案”，却不询问这些条件，恰好违背了系统设计的本质。

### 1.6 难点四：没有唯一标准答案

没有唯一答案，不等于没有评价标准。系统设计往往要同时考虑延迟、可用性、一致性和可维护性等目标，并满足吞吐、存储容量、成本与合规性等硬约束。评价时要区分两件事：
...
优化必须针对具体指标和瓶颈，否则无法判断是否有效。优化过程应从目标和基线设计出发，经过压力分析、瓶颈定位与局部改进，再重新评价结果。
...
开放式问题允许多个方案，前提是它们满足双方约定的功能需求、非功能需求和约束。若有多个方案满足条件，再比较成本、复杂度、演进性等软目标。

这里的关键是评价顺序：**先判断是否满足约定目标，再比较方案优劣。** 不先约定目标，就无法有意义地争论 SQL 还是 NoSQL、同步还是异步、强一致还是最终一致。
...
策略、知识和沟通都不可替代，任何明显短板都会限制整体表现：知识很多但无法解释，面试官无法检查；表达流畅但设计不满足规模，也不能靠沟通补救。
...
可以用资源效率建立简化直觉。若工作负载增长一倍，投入资源也接近同比增加，且延迟、错误率等仍在目标范围内，系统具有较好的扩展特性。若资源增加十倍只能支持两倍流量，或协调开销导致性能下降，扩展性就较差；批处理、缓存升温或固定开销被摊薄，也可能带来阶段性的超线性收益。真实系统还必须同时观察延迟、可用性、成本和数据规模，不能仅凭吞吐判断扩展性。
...
例如，若有 1000 万日活用户，每人每天平均发送 20 条消息，则平均消息写入率约为 2315 条/秒；若峰值是平均值的 5 倍，则峰值约为 11575 条/秒。
...
系统化方法要求每个关键决定都有来源，并能说明对应的需求或风险、设计决策、预期效果、代价与验证方式。
...
例如，每日新增数据可以用日活用户数、人均操作数和单条数据大小做粗略估算。
...
面对任何系统设计问题，可以依次澄清目标与边界、量化规模和约束、建立简单完整的基线、检查瓶颈与失败模式、比较并选择机制、解释权衡并验证，最后总结与演进。

这个顺序的重点不在背诵八个词，而在保持因果方向：**先有目标，后有设计；先有瓶颈，后有优化；先说明依据，再做技术选择。**
