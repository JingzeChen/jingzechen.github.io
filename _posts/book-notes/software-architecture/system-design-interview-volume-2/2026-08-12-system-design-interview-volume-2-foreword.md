...
为了把这些简短论述讲透，本文会加入数值示例、流程清单和图表。它们用于展开作者的方法论，不是 Foreword 原文声称存在的万能答题模板。
...
估算时，可以先用当前峰值请求率和预计增长倍数得到目标峰值，再除以单个实例保留安全余量后的稳定吞吐，并向上取整。例如峰值为 120,000 QPS，每实例安全吞吐为 4,000 QPS，则至少需要 30 个实例。
...
可以把这个过程看成逐步缩小解空间：每确认一个真正会影响设计的问题，就排除一批不合适的方案。提问的价值不在数量，而在**信息增益**。
...
因此，架构不是由题目名称唯一决定的，而是需求、规模、约束和优先级共同作用的结果。这些条件发生变化时，应重新检查架构是否仍然合理。
...
判断方案时，应在所有满足硬约束的架构中，根据业务优先级比较延迟、可用性、一致性、可维护性、成本和复杂度。实际面试不会精确计算这些指标，因为它们难以归一化，优先级也未必能量化。评价顺序是：
...
这体现了一个重要原则：新增机制的收益应大于它引入的复杂度与风险。没有需求、数字或瓶颈支撑的复杂组件，只会增加新的数据语义、成本和失败方式。
...
它比“这里加 Redis”更完整，因为建立了从问题、决策、代价到验证的完整解释链。
...
设计可以有多个，但不满足功能需求、非功能需求和约束的方案，不能因为题目开放就被视为合理。
...
对串联系统，一个环节吞吐不足就会限制整体吞吐。在没有并行扩展和缓冲改善的情况下，整体稳定吞吐受处理能力最低的环节限制。
...
若每条消息连同元数据平均占 1 KB，则未考虑副本和索引的每日新增数据约为 4 TB。
...
策略、知识、沟通、验证与迭代能力共同决定面试表现，任何明显短板都会限制整体表现。知识再丰富，如果完全不澄清目标，仍可能精确地解决错误问题；流程再熟练，如果不了解分片和复制的代价，也无法完成可靠设计。
