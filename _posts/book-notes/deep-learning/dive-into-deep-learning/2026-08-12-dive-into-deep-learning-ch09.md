...
> 本章既是 RNN 入门，也是序列建模入门：先讨论序列为何不能当作独立表格行处理，再把原始文本变成词元索引；随后从概率链式法则建立语言模型，以困惑度评价模型；最后推导隐藏状态递推、字符级 RNN、梯度裁剪和时间反向传播（BPTT）。第 10 章才进入 GRU、LSTM、深层/双向 RNN 和机器翻译等现代结构。
