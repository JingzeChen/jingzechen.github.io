...
因此，更符合实际的理解是：一个人在特定领域、特定时刻的表现，不只取决于练习时数，还受任务质量与难度、反馈与纠错、已有领域知识、认知和身体条件、环境资源、动机与注意力，以及参与高质量实践的机会影响。不同领域的作用机制也不同，时数不能单独承担全部解释。
...
组块化能有效扩展任务中的信息处理量。专家未必拥有成倍扩大的通用“内存槽位”，而是凭借领域知识改变了信息的组织单位。
...
固定阈值主张实际上要求所有领域、所有个体都恰好在 1 万小时达到相应标准。
...
真实学习曲线通常并非直线。初期可能快速进步，随后进入平台；更换方法后再次跃升；疲劳、受伤或错误自动化还可能使表现下降。练习的边际收益取决于学习者当前水平、任务难度、反馈、恢复和既有表征。
...
经过大量正确练习，常见结构的识别和动作执行会加速，释放有限的工作记忆去处理更高层问题。
...
一次最小训练循环包括尝试、测量偏差、诊断原因、改变一个关键变量，再次尝试。
...
书本擅长传递可陈述的概念和规则，却很难穷举现实中的全部条件组合。一条规则可能要求在某种条件下采取某种行动，但现实中的难点常常不在记住行动本身，而在判断：
...
迁移能力形成于两种力量的平衡：它既需要具体情境中的深度经验，也需要跨情境比较与抽象。
...
这里的**周期**不是把相同动作再重复一遍，而是一个能产生信息并改变下一轮行为的闭环：它从目标和行动出发，根据反馈反思，并据此修正下一轮的目标与行动。
