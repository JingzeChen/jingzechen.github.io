...
集中练习往往让当场表现很高，但它只能提供有限证据。真正要判断是否学会，必须看延迟测试和迁移测试。

本章最关键的反常结果就是：有些方法在练习阶段看起来更好，到了延迟测试反而更差。几何题实验的交叉结果正说明了这一点，迫使我们放弃“练习时最好就是学得最好”的直觉。

### 先区分五种容易混淆的练习结构

| 练习结构 | 主要操纵维度 | 安排示例 | 典型风险 |
| --- | --- | --- | --- |
| 集中练习 | 同一内容的时间密度 | 同类题连续做，间隔很短 | 依赖短时激活，快速遗忘 |
| 间隔练习 | 同一内容的时间距离 | 同类题分散到不同天练 | 眼前进步慢，可能检索失败 |
| 段落练习 | 类型被分块、固定成段 | 先整段做完题型甲，再做题型乙 | 不必判断当前该用什么方法 |
| 穿插练习 | 相关类型交替顺序 | 相关题型交替出现 | 初学者感觉混乱、正确率下降 |
| 多样化练习 | 同一能力的条件变化 | 同一技能在不同参数和情境下练 | 变化过宽可能淹没稳定规律 |
...
- 固定组始终在 3 英尺练；
- 多样组在 2 英尺和 4 英尺之间交替练；
- 最终测试统一在 3 英尺。

#### 1.3 为什么多样组可能更好

固定组反复调用同一力度与轨迹，容易形成适配 3 英尺练习情境的单一动作。多样组每次都要根据距离调整力度、角度和动作，并从结果反馈中学习“条件变了，动作也要跟着变”。

当测试距离位于 2 英尺与 4 英尺之间时，多样组更容易做出中间距离所需的调整，而固定组缺少这类调整练习。
...
集中练习中，相邻练习间隔很短；间隔练习中，相邻练习间隔足够长，长到学习者下一次需要重新提取，而不是无意识复述。
...
实践上可以这样理解：最合适的间隔取决于材料难度、当前掌握程度和你希望保持多久，没有一刀切的固定天数。保持目标越远，后续间隔通常可以逐渐拉长。
...
总题量可以完全相同，区别只在排序。段落安排是先把某一类连续做完再换下一类；穿插安排是在各类都还没做完时交替切换。
...
真实解题不是只执行策略，而是先辨识题目属于哪一类，再选择对应方法，最后执行求解。段落练习主要训练“执行”，穿插练习还训练“辨识”和“选择”。
...
目标不是记住每个条件下的孤立答案，而是学会在条件变化时如何调整行动，并提取跨条件稳定的原则。
...
段落练习主要训练“已知方法后的执行”，真实任务却要求先判断题目属于哪一类，再从多个方法中选出当前该用的一个。
...
因此分类时要综合多个线索，并按诊断价值权衡，而不是死背某一条“只要出现这个特征就一定属于某科”的定义。也就是说，学习目标是形成可比较、可加权的分类判断，而不是套一条单一规则。
...
换句话说，复合型知识至少包含五层：事实性知识、概念关系、操作程序、何时使用的条件性知识，以及监控与调整能力。这些部分共同工作，才会在真实场景里形成稳定表现。
...
这叫**迁移适切加工**或训练—测试匹配原则。它不要求训练与考试表面完全相同，但要求关键判断和操作过程相似；相似度越高，迁移通常越好。
...
这练的是条件性规则：不同情境出现时，要迅速选出对应动作。多种环境训练越充分，比赛中遇到新组合时越可能找到相近模型并调整。
...
评价学习效率不应只看单位时间接触了多少内容，还要看延迟后还能正确提取并应用多少内容。间隔练习当下输入速度较慢，却可能提高这种“可用效率”。
...
先完成题型甲全部练习，再完成题型乙，并不是穿插。真正穿插要求在题型甲尚未练完时转到题型乙，再回来检索题型甲。
...
| 时间 | 训练内容 | 主要原则 |
| --- | --- | --- |
| 第 0 天 | 学题型甲、题型乙的基本模型，各做少量题 | 初始理解 |
| 第 1 天 | 题型甲与题型乙混合，反馈后解释差异 | 穿插、检索 |
| 第 3 天 | 加入题型丙并改变参数 | 间隔、多样化 |
| 第 5 天 | 三类题混合随机分类，不立即告知题型 | 辨识、选择 |
...
15. 从 `20%` 到 `63%` 为什么既是高 43 个百分点，又是以 `20%` 为基准相对提高 `215%`？
...
21. 十步工序能否直接按“跳步顺序”用于真实生产？训练中应怎样穿插？
...
15. 绝对差是 63% 减 20%，即 43 个百分点；相对基线增长是用 43 除以 20%，即 215%。
