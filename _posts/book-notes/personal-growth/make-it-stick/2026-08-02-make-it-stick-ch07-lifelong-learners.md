...
设个体在领域 $d$、时间 $t$ 的表现为 $P_d(t)$，可概念化为：

$$
P_d(t)=F_d\bigl(G,N(t),K_d(t),Q_d(t),M(t),E(t),H(t)\bigr).
$$
...
- $M(t)$：动机、自律和信念；
- $E(t)$：教育、家庭、文化与资源环境；
- $H(t)$：睡眠、营养、健康和压力等身体条件；
- $F_d$：领域特定的综合关系。

公式不是原书的定量模型，而是提醒我们：表现不由单一变量决定，且不同领域有不同的发展函数。
...
实验显示的关键不是“好孩子天生忍耐”，而是策略：

$$
\text{诱惑强度}\downarrow\quad\text{或}\quad\text{目标表征}\uparrow
\Rightarrow\text{等待概率上升}.
$$

遮住棉花糖、转移注意和重新想象，都在改变决策环境。
...
其中 $G\times E$ 不是普通乘法，而表示基因—环境交互：同一环境对不同遗传背景作用可能不同，同一遗传倾向在不同环境中也可能产生不同表现。
...
有意识目标行为与习惯行为会更多依赖不同神经网络。重复的动作与认知序列可被组块化，像计算机宏一样作为一个单元触发。
...
组块 $M$ 降低逐步决策成本，支持打字、演奏、棋局识别和运动反应。若训练序列含错误，错误也会被一并组块化。
...
学习对应真实神经变化，但不能把复杂学习压缩成“多长几个突触”。更可靠的层次是：

$$
\text{有效经验}\rightarrow\text{网络活动改变}\rightarrow\text{结构/功能适应}\rightarrow\text{表现变化}.
$$
...
$$
\text{略强好奇心}\rightarrow\text{探索更多}\rightarrow\text{知识增加}
\rightarrow\text{更容易理解新事物}\rightarrow\text{探索更多}.
$$
...
但主观自信不能被报告成 IQ 提高。应分别测量：

$$
\Delta S=\text{主观自信变化},\qquad
\Delta O=\text{客观未练任务变化}.
$$
...
$$
\text{失败}\xrightarrow{\text{固定归因}}\text{无助与回避},
$$

$$
\text{失败}\xrightarrow{\text{可控归因}}\text{换策略、求助与继续练习}.
$$
...
$$
DP=Goal+Stretch+Feedback+Correction+Repetition.
$$
...
$$
\text{线索}\rightarrow\text{识别模式}\rightarrow\text{预测}\rightarrow\text{行动}.
$$
...
图像通常比孤立词更具辨别性；空间路线提供顺序；夸张互动增加独特性。

$$
\text{目标信息}\leftrightarrow\text{形象}\leftrightarrow\text{位置}\leftrightarrow\text{路线顺序}.
$$
...
帕特森发现，自己能记山峰名称，却看不到山脉、河谷和生态关系。助记术只有在以下流程中价值最大：

$$
\text{理解}\rightarrow\text{提炼结构}\rightarrow\text{建立助记索引}\rightarrow\text{检索与应用}.
$$
