...
- 先看长期保持，而不是只看当天练习是否顺畅；
- 再看能不能迁移到新情境，而不是只会做原题；
- 同时计算实施成本，包括时间、注意力、设备条件和组织支持；
- 最后比较“长期可用结果”和“实际投入”是否匹配。

这个框架提醒我们避免两种误判：把短期流畅当作真正掌握，或忽视现实执行成本，设计出无法长期坚持的方案。
...
有效的解释顺序是：困难先暴露缺口，再通过反馈与练习推动能力变化。
...
当复习时间有限时，优先分配给缺口更大、风险更高、离考试或应用更近的内容。先补最影响结果的短板，再扩展到次要内容；这不是机械计时，而是有依据地排序。

#### 3.7 为什么比重读好

重读主要提高材料熟悉度，检索直接练未来任务所需过程：看到问题线索后，从长期记忆中搜索并组织出可用答案。
...
目标是让每次检索都需要努力但不过载：既能逼出主动回忆，又不会连续失败到放弃。
...
例如，段落练习可能是同一题型连续做完再换下一种；穿插练习则是在同一轮中交替出现不同题型，迫使学习者先判断当前题目属于哪一类，再选择方法并执行。
...
真实问题通常要求三个连续动作：先识别类型，再选择规则，最后执行。
...
因此要主动把工作组织成闭环：先接任务并尝试，再拿反馈复盘，最后把修正后的做法放回下一次应用。
...
没有初稿，意识就没有可修改对象。初稿的价值在于把模糊想法外化成可检查文本，再通过反馈与修改把模型逐步变清晰。
...
更实用的判断是同时看四个因素：学生感受到的价值、执行成本、规则明确性和成绩权重。四者任一项失衡，都可能导致参与率下降。
...
总结表不是装饰，而应表达可解释的因果结构。可以用有方向的连线标出促进、抑制和反馈关系，并且能逐条解释每条连线为什么成立。
...
这就是课程层面的间隔与穿插：先让学生初识概念，隔一段时间再次遇见，在更高要求下深化，最后进入跨主题组合应用。
...
实际执行时要把长期愿景逐级拆成年度指标、月度行为和每日动作，确保目标能落到可观察任务上。
...
书面标准保证质量一致。新员工由老员工带，按“先说明、再示范、然后主动执行、最后反馈复盘”的顺序训练。
...
一个起始计划：当天、第 1 天、第 3 天、第 7 天、第 14 天、第 30 天。之后根据回忆结果增减，不把这组日期当作固定配方。
