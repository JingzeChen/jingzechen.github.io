...
“适合自己”不是一句感受，而是四件事同时对上：学习目标是什么、内容本身需要怎样呈现、你当前具备哪些知识与能力、这套方法能否在延迟测验和迁移任务里真的见效。

所以“我喜欢”当然重要，它会影响投入和坚持，但它只是判断标准中的一部分，不能单独决定方法是否有效。
...
一个清晰的闭环是：先提出问题，再去探索并行动，然后根据结果反思，最后更新下一轮规则。
...
他从“资产会自动带来现金流”的简单想法，更新为更完整的核算：净收益要把租金减去融资成本、维修支出、空置损失和管理成本。
...
如果一只股票从 100 跌到 50，先亏掉的是一半；要从 50 回到 100，就得再涨 50，而这 50 相对 50 本身正好是 100%。
...
其他人因破产恐惧低价出售，布鲁斯的优势不是胆子大，而是信息和模型更精细。他会同时核对可变现资产、总负债、不同债券的清偿优先级、当前市场价格，以及在破产处置中大致能收回多少。只有把这些拼在一起，才看得出某类债券是否被错杀。
...
可靠掌控感是把注意力放在可控行动上，而不是相信所有结果都由自己决定。
...
1. 用明确方法先把学生分成视觉偏好者和听觉偏好者；
2. 在每一种偏好组内部，再随机分配视觉教学或听觉教学；
3. 内容相同；
4. 所有人接受相同、客观的最终测验；
5. 检查是否出现交叉匹配优势。

要支持匹配假说，必须出现方向相反的优势：视觉偏好者在视觉教学下比在听觉教学下学得更好，同时听觉偏好者在听觉教学下也比在视觉教学下学得更好。

如果只是视觉教学对所有人都更有效，或视觉偏好者整体基础更强，都不能证明“按偏好匹配”这个因果主张。
...
解决问题时，流体智力与晶体智力是协作关系：前者负责在陌生情境中推理和重组，后者提供事实、概念和程序化经验；两者都要放在具体任务情境里才会表现出来。

有推理却缺领域知识，会反复从零开始；知识丰富却不会调整，也难解决新问题。

### 3. 传统智商比率

早期智商曾用“心理年龄和实际年龄的比值”来表示。比如一个 8 岁儿童，如果完成任务的水平接近当时常模里的 10 岁儿童，按旧算法会记成 125。
...
一个人的表现总是能力、机会、知识基础、练习强度和文化价值共同作用的结果。
...
最直接的做法是比较干预前后在同一能力上的表现差值，也就是看训练后比训练前到底提升了多少。
...
可以把知识结构理解为一个由“概念节点”和“节点关系”组成的网络，而且不同关系有轻重之分。

低级构建者的问题不一定是概念太少，更多时候是关系混乱、主次不清，或噪声信息太多。
...
规则学习者面对新问题时，会先判断它对应哪条规则，再按规则生成方案。

案例学习者更常先在记忆里找“看起来像”的旧案例，再沿用当时做法。风险在于只看表面相似而忽略结构差异，导致误用。
...
通用规则是：每一路径只施加不会造成损伤的小强度，让多路力量在目标点汇聚，最终达到足以解决问题的总强度。
...
它至少同时包含事实、规则、结构、条件判断和实践反馈，缺一项都容易在真实情境里失手。
