...
可把一项知识的实际可用性理解为三个条件同时满足：记忆本身要存得住，线索出现时要提得出，还要能对上当前任务。三者任何一环掉链子，知识都很难在关键时刻真正可用。
...
埃伯索尔德回家后会思考缝针应疏还是密、线应怎样安排、不同调整会造成什么后果。这个过程的要点是：把一次具体事件拆成可复查的事实、可解释的原因、可替换的做法，再在后续实践中验证。关键不是回忆得多生动，而是形成下一次可检验的预测与动作。
...
这条成长路径通常是：先能说清步骤和理由，再能稳定做出来，最后在合适线索出现时几乎不假思索地执行。它不是单向通道。专家仍要能回到解释层，诊断问题并修改程序。

#### 3.4 自动化为什么能提升高阶表现

工作记忆资源有限。基础操作越不熟练，越会占用注意力，留给异常判断和策略选择的空间就越小。自动化的价值在于把基础步骤做得更省心，从而把有限资源让给真正关键的诊断、监控和决策。
...
遗忘常见的时间轨迹是先快后慢：刚学完的阶段掉得更快，随后下降变缓。不同人、不同材料、不同学习条件下，下降速度会明显不同，不存在一个放之四海而皆准的统一曲线。
...
比较检索效应时，要把“做过检索练习”的学习者与“只做重新学习”的学习者放在可比条件下，再观察延迟成绩。如果检索组整体更好，就说明检索在该条件下带来了额外收益；收益大小则会随材料、测试方式、反馈、间隔和人群变化。
...
把重复检索拉开间隔时，要做到两点平衡：间隔要足够长，长到学习者需要重新搜索记忆；但也不能长到几乎完全提不出来、又拿不到反馈。有效间隔来自这种平衡，而不是固定天数。
...
因此更准确的关系是：高阶表现依赖可提取的知识，也依赖策略、判断和迁移练习。测验会不会变成死记硬背，取决于测什么、怎样反馈、是否要求迁移，而不是“测验”这个名称本身。
...
如果把“不中途测验”的记忆轨迹与“中途插入测验”的轨迹对比，常会看到后者在测验之后保持得更好。也就是说，测验会改变随后记忆状态，而不是中性地读取它。
...
在该研究中，集中学习者两天后大约忘掉初次测试中记住内容的一半；检索练习者同期只忘掉约 13%。这说明评价学习策略不能只看第一次成绩，还要看后续衰退幅度。
...
一周后，一次测验组为 39%，无测验组为 28%，前者高 11 个百分点。若以 28% 为基准，39% 大约是提高四成（约 39.3%）。两种说法回答的是不同问题，不能混用。
...
这个设计的关键对比是：一类内容要求学生主动检索 3 次，另一类内容只是额外呈现 3 次但不要求检索。
...
两类内容平均相差 13 分。这不是说每位学生都提高 13 分，而是特定内容条件下的平均差异。研究还发现效应在学年考试后持续约 8 个月，支持检索的长期价值。
...
每次测验既巩固旧知识，又让后续课程建立在更牢基础上。用通俗的话说，下一阶段能用的知识，取决于现在保住了多少旧知识、这段时间学到了多少新知识，以及遗忘和干扰带走了多少。“复利”只是比喻：它强调早期保持做得好，后续理解成本会更低、累积会更顺，而不是说知识会按固定利率自动增长。
...
可以把它理解成倒 U 形关系：太容易、几乎不需要检索时，长期收益偏低；难度适中、需要努力且最终能答出来时，收益最高；太难到持续失败又没有反馈时，收益会再次下降。这个关系会随人和任务变化，最重要的前提仍是原书强调的“真正发生了检索”。
...
作答时给出 0 到 1 的信心值，核对后记录每题对错。之后按整组题目统计“自信和实际表现的平均偏差”，偏差越小，说明自我判断越准。重点关注“高信心但答错”的未知未知。
...
15. 从 `28%` 到 `39%` 是增加 11% 还是 11 个百分点？若按 28% 为基准，相对提升大约是多少？
...
15. 增加 11 个百分点；若按 28% 为基准，相对提升约 39.3%。
